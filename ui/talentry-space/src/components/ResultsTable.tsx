  ...
  <section className="card overflow-hidden">
    ...
    <div className="max-h-[60vh] overflow-auto">
      <table className="w-full text-sm border-collapse">
        <thead className="sticky top-0 bg-ink-900/95 backdrop-blur text-bone-400">
          ...
          {rows.map((r) => {
            ...
            return (
              <tr
                key={r.candidate_id}
                onClick={() => onSelect(r)}
                className={`border-t hairline cursor-pointer transition-colors ${
                  selected ? "bg-bone-50 text-ink-950" : "hover:bg-ink-800"
                }`}
              >
                ...
                <td className="px-4 py-3 text-bone-200 group-hover:text-inherit">{r.reasoning}</td>
              </tr>
            );
          })}
        </tbody>
